...
type Props = {
  ...
  onConfirm: (data: { amount: number; label: string; bucketId: string; intent: "want" | "need"; readinessTag: ReadinessTag | null }) => void;
  onSleepOnIt: (data: { amount: number; label: string; bucketId: string; readinessTag: ReadinessTag | null }) => void;
...
export function PurchaseFlow({
  ...
  const finalize = (bId: string) => {
    setStep("confirm");
    setTimeout(() => {
      onConfirm({ amount: amt, label: label.trim(), bucketId: bId, intent: intent!, readinessTag: readiness });
    }, 1000);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md rounded-3xl p-0 overflow-hidden">
        ...
        {step === "amount" && (
          <motion.div key="amount" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.2 }}>
          <div className="p-6 animate-fade-in">
            <p className="text-xs uppercase tracking-[0.15em] text-muted-foreground">Step 1 of 3</p>
            ...
            <div className="mt-6 space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Input id="lbl" placeholder="e.g. Blue Bottle Coffee" value={label} onChange={(e) => setLabel(e.target.value)} className="h-12 rounded-xl" />
              </div>
              <Button disabled={!amtValid} onClick={() => setStep("intent")} className="w-full h-12 rounded-xl text-base">
                Continue
              </Button>
            </div>
          </div>
        ...
        {step === "intent" && (
          <motion.div key="intent" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.2 }}>
          <div className="p-6 animate-fade-in">
        ...
        {step === "readiness" && (
          <motion.div key="readiness" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.2 }}>
          <div className="p-6 animate-fade-in">
            <p className="text-xs uppercase tracking-[0.15em] text-muted-foreground">Step 3 of 4</p>
        ...
        {step === "bucket" && (
          <motion.div key="bucket" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.2 }}>
          <div className="p-6 animate-fade-in">
            <p className="text-xs uppercase tracking-[0.15em] text-muted-foreground">
              {intent === "want" ? "Step 4 of 4" : "Step 3 of 3"}
            </p>
            <h2 className="mt-1 text-2xl font-semibold">Which bucket?</h2>
            <p className="mt-1 text-sm text-muted-foreground">{formatCurrency(amt)} · {label}</p>
                    ...
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="text-xs text-muted-foreground tabular-nums">
                          {formatCurrency(b.balance)} → <span className={cn(negative && "text-destructive")}>{formatCurrency(after)}</span>
                        </div>
                      </div>
                    </div>
                    {b.ownerType === "child" && (
        ...
        {step === "intercept" && (
          <motion.div key="intercept" initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.25 }}>
          <div className="p-7">
            ...
            <p className="mt-2 text-center text-sm text-muted-foreground max-w-xs mx-auto">
              This is a <span className="font-medium text-foreground">{formatCurrency(amt)}</span> want.
              Feel it for a moment — then decide.
            </p>
            <div className="mt-6 grid gap-2">
              <Button onClick={sleepOnIt} className="h-12 rounded-xl text-base bg-primary hover:bg-primary/90">
        ...
        {step === "askParent" && (
          <motion.div key="askParent" initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }}>
          <div className="p-7">
            <div className="mx-auto h-14 w-14 rounded-full bg-accent flex items-center justify-center">
            ...
            <div className="mt-6 grid gap-2">
              ...
              <Button variant="outline" onClick={() => setStep("bucket")} className="h-12 rounded-xl">
...
function IntentButton({
  ...
  return (
    ...
    >
      ...
      <span className={cn("text-xs", selected ? "text-background/70" : "text-muted-foreground")}>{hint}</span>
    </button>
  );
}